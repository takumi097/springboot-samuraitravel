...
public class AdminHouseControllerTest {
	...
	public void 一般ユーザーとしてログイン済の場合は管理者用の民宿一覧ページが表示されずに403エラーが発生する() throws Exception {
		mockMvc.perform(get("/admin/houses"))
	...
	@Test
	@WithUserDetails("hanako.samurai@example.com")
	public void 管理者としてログイン済みの場合は管理者用の民宿ページが正しく表示される() throws Exception {
		mockMvc.perform(get("/admin/houses/1"))
				.andExpect(status().isOk())
	...
	@Test
	public void 未ログインの場合は管理者用の民宿ページからログインページにリダイレクトする() throws Exception {
	...
	@Test
	@WithUserDetails("taro.samurai@example.com")
	public void 一般ユーザーとしてログイン済みの場合は管理者用の民宿登録ページが表示されずに403エラーが派生する() throws Exception {
		mockMvc.perform(get("/admin/houses/register"))
		.andExpect(status().isForbidden());
	}
