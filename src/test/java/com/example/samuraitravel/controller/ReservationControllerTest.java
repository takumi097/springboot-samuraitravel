...
public class ReservationControllerTest {
	...
	*/
	...
	public void ログイン済みの場合は会員用の予約一覧ぺージ時が正しく表示される() throws Exception {
		mockMvc.perform(get("/reservations"))
	...
	public void 未ログインの場合は予約フォームを送信せずにログインページにリダイレクトする() throws Exception {
		mockMvc.perform(post("/houses/1/reservations/inout").with(csrf())
	...
	public void 未ログインの場合は予約内容の確認ページからログインページにリダイレクトする() throws Exception {
		//セッションを作成し、ReservationDTOオブジェクトを保持する
