...
public class HouseControllerTest {
	...
	@Test
	public void 未ログインの場合は会員用の民宿ページが正しく表示される() throws Exception {
	...
	public void ログイン済みの場合は会員用の民宿ページが正しく表示される() throws Exception {
		mockMvc.perform(get("/houses"))
		.andExpect(status().isOk())
		.andExpect(view().name("houses/index"));
	}
	...
	@Test
	@WithUserDetails("taro.samurai@example.com")
	public void ログイン済み帆場合は会員用の民宿詳細ページが正しく表示される() throws Exception {
		mockMvc.perform(get("/houses/1"))
		.andExpect(status().isOk())
		.andExpect(view().name("houses/show"));
	}
